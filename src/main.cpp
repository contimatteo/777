    ...
    if((valore_casella==7) || (valore_casella==8) || (valore_casella==11))
    {
        ...
        switch(valore_casella)
        {
            ...
            case 8:
            {
                ...
                eroe.resetPosizionePersonaggio(lista_torre, stanza);
                nemici.ricreaNemici(lista_torre, eroe, grafica.array_posizioni_consentite, grafica.lunghezza_array, piano, stanza);
                nemici.aggiornaAttaccoNemici(-50);
                break;
            }
            // scala a salire
            case 11:
...
int main()
{
    ...
    // nascondo la finestra per mostrare il menù iniziale
    Gioco.setVisible(false);

    // Instanzio la lista torre (contenente tutte le stanze divise in piani)
    ...
    // Instanzio la mappa
    Grafica grafica(0);
    // instanzio l'array delle posizioni consentite (caselle dove c'è l'erba) per la prima stanza al primo piano
    ...
    while (Gioco.isOpen())
    {
        ...
        // Catturo gli eventi
        sf::Event event;
        // Processo in loop gli eventi generati
        ...
        if ((sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) && muovi_personaggio)
        {
            ...
            // il nemico si muove o attacca di conseguenza
